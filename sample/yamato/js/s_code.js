...
var s_account="Ymtyamatohdcojp,ymtyamatomst"
...
s.linkInternalFilters="javascript:,yamato-hd.co.jp,.kuronekoyamato.co.jp" 
...
function s_doPlugins(s) {
	...
	s.campaign=s.getValOnce(s.getQueryParam('cid,banner_id',':',location.search),'s_campaign');
	s.eVar20=s.getValOnce(s.getQueryParam('cid,banner_id',':',location.search),'s_eVar20');
	
	/* Set s.prop1 s.eVar1 s.prop26 URL */
	s.prop1=s.eVar1=s.prop26="D=g"
	...
	if(length>=5){
		s.channel=pathArray[1];
		s.prop3=s.eVar3=pathArray[1]+"/"+pathArray[2];
		s.prop4=s.eVar4=pathArray[1]+"/"+pathArray[2]+"/"+pathArray[3];
		s.prop5=s.eVar5=pathArray[1]+"/"+pathArray[2]+"/"+pathArray[3]+"/"+pathArray[4];
	}else if(length=="4"){
		s.channel=pathArray[1];
		s.prop3=s.eVar3=pathArray[1]+"/"+pathArray[2];
		s.prop4=s.eVar4=s.prop5=s.eVar5=pathArray[1]+"/"+pathArray[2]+"/"+pathArray[3];
	}else if(length=="3"){
		s.channel=pathArray[1];
		s.prop3=s.eVar3=s.prop4=s.eVar4=s.prop5=s.eVar5=pathArray[1]+"/"+pathArray[2];
	}else if(length=="2"){
		s.channel=s.prop3=s.eVar3=s.prop4=s.eVar4=s.prop5=s.eVar5=pathArray[1];
	}else{
		s.channel=s.prop3=s.eVar3=s.prop4=s.eVar4=s.prop5=s.eVar5="/";
	}
	
	/* set Site and Page Name for MST */
	s.eVar27=s.prop27="YHD";
	s.eVar28=s.eVar27+"_"+document.URL.replace( /(\?|#).*$/, '' );
	s.prop28=s.prop27+"_"+document.URL.replace( /(\?|#).*$/, '' );
	s.eVar29=s.eVar27+"_"+document.title;
	s.prop29=s.prop27+"_"+document.title;
	s.eVar32=s.eVar27+"_"+s.channel;
	s.prop32=s.prop27+"_"+s.channel;
	...
	s.prop6=s.eVar6=s.prop31=s.eVar31=sc_day+"_"+sc_hour;
	
	/* Plugin : getNewRepeat 1.0 */
	s.prop10=s.eVar2=s.prop30=s.eVar30=s.getNewRepeat();
	
	/* Set s.eVar10 Variables on E-mail campaign value */
	s.eVar10=s.getValOnce(s.getQueryParam('mid',':',location.search),'s_eVar10');
	
	/* Set s.eVar11 Variables on Internal campaign value */
	s.eVar11=s.getValOnce(s.getQueryParam('xid',':',location.search),'s_eVar11');
	
	/* Set s.eVar15 Variables on All Param value */
	s.eVar15=s.getValOnce(s.getQueryParam('cid,banner_id,mid,xid',':',location.search),'s_eVar15');
	
	/* s.crossVisitParticipation 1.2 for campaign tracking */
	s.eVar12 = s.crossVisitParticipation(s.campaign,'s_cpm1','180','10','>','',0);
	s.eVar21 = s.crossVisitParticipation(s.eVar15,'s_cpm2','180','10','>','',0);
	
	/* s.crossVisitParticipation 1.2 for path tracking */
	s.eVar16 = s.crossVisitParticipation(s.channel,'s_path1','180','10','>','',0);
	s.eVar17 = s.crossVisitParticipation(s.eVar3,'s_path2','180','10','>','',0);
	s.eVar18 = s.crossVisitParticipation(s.eVar4,'s_path3','180','10','>','',0);
	s.eVar19 = s.crossVisitParticipation(s.eVar5,'s_path4','180','10','>','',0);
	...
	var chkref=s.getEngineAndKW('p','','cid,banner_id');
	if(s.campaign||chkref=="Natural"){
		if(chkref=="Natural"){
			var ntrl=s.getValOnce((s.getEngineAndKW('d')+":"+s.getEngineAndKW('k')),'s_ntrl');
			s.eVar13=s.crossVisitParticipation(ntrl,'s2_ntrl','180','10','>','',0);
		}
		...
		s.eVar14=s.crossVisitParticipation(camntrl,'s2_camntrl','180','10','>','',0);
	}
...
s.visitorNamespace="yamatohd"
s.trackingServer="yamatohd.d1.sc.omtrdc.net"
